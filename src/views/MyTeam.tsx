...
export function MyTeam() {
  ...
  const { advice, alerts, needs } = analysis;
      ...
      <div className="grid cols-2">
        ...
        <div className="card">
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Player</th><th className="right">Proj</th><th className="right">Value</th></tr>
              </thead>
              <tbody>
                {bench
                ...
                {(roster.reserve ?? []).map((id) => (
                  <tr key={id}>
                    <td><PlayerCell playerId={id} /> <span className="muted small">IR slot</span></td>
                    <td className="right num">{projPts(id).toFixed(1)}</td>
                    <td className="right num">{values[id]?.value ?? 0}</td>
                  </tr>
                ))}
