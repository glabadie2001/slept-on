...
src/
  ...
  lib/
    ...
    mockDraft.ts     seeded mock-draft engine: need-weighted CPU picks, interactive
                     or auto picks for you, undo, hauls, batch runs (forced picks,
                     sampled policy) keeping every timeline
    mockAnalysis.ts  batch read-outs: availability heatmap, value × availability
                     targets, positional runs, typical roster, timeline scoring,
                     best/median/worst, "what had to go right"
    timelines.ts     decision-conditioned futures per candidate pick, chained into
                     the season simulator for playoff/title odds
...
- [ ] Fresh redraft boards bundled offline (FantasyPros redraft ECR / ADP snapshots) — today redraft
      relies on the live FantasyCalc + Sleeper ADP feeds, which need the browser to reach them
